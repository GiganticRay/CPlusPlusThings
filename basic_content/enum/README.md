...
有时我们希望某些常量只在类中有效。 由于#define 定义的宏常量是全局的，不能达到目的，于是想到使用 const 修饰数据成员来实现。而 const 数据成员的确是存在的，但其含义却不是我们所期望的。
...
self: enum 再次加深了我对类中 const 变量的理解，类中的 const 变量，并不是为了给这个类的所有对象一个固定的值，而是说，该类型的所有对象实体被初始化之后，这个 const 变量的值就不会被改变了。根据这个应用场景，**类中 const 变量就不能在类的申明时初始化**, 而应该在构造函数处初始化。

slef: 同时，为了实现使所有类实例都具有一个常量，可以选择用类中枚举来做。