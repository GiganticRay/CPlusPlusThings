...
int Apple::i = 1; 	// 使用类外的类名和范围解析运算符 显示初始化
...
self: 那么我们是否可以在 if 块的外边儿调用 if 块内部弄的 static 对象呢？
...
self: conclusion, 那我们什么时候用静态呢：针对普通变量，我们想要其生成于整个函数的生存周期，即仅在第一次被初始化值。针对类中的变量，我们想要其在所有的该类的实例对象中畅行，所以只能在类实例调用前显示初始化。针对类中的函数，有点像该类的 help 函数，其不能够调用其他非 static 成员。
